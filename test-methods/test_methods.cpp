...
void test_method(
    ...
    for (int i = 0; i < (int)start_points.size(); ++i) {
        ...
        fout_txt << "\t?? ????? #" << i << " ???????? ? " << best_point.type << " #" << best_id
            << " (????????: " << std::setw(8) << iter_counter << ") ";
        if (best_eps_f == 1e9) {
            fout_txt << "??? ????? ????? :(\n";
        } else {
            fout_txt << ", ?????????? = " << std::fixed << std::setprecision(24) << std::setw(30) << best_eps_f << std::endl;
        }
        fout_txt << "\t\t                ????????? ?????: " << p << std::endl;
        fout_txt << "\t\t  ?????????????? ????? ????????: " << best_point.x << std::endl;
        fout_txt << "\t\t      ?????????? ????? ????????: " << x << std::endl;
        fout_txt << "\t\t?????????????? ???????? ???????: " 
...
void test1(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 1 -----------------------------------\n\n";
    fout_txt << "01. ??????? ???????: f(x,y) = 1+x+y-xy+x^2+y^2, ??????? ???????????? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test2(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 2 -----------------------------------\n\n";
    fout_txt << "02. ??????? ???????: f(x,y) = 1+7x+5y+0.5xy+3x^2+y^2, ??????? ???????????? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test3(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 3 -----------------------------------\n\n";
    fout_txt << "03. ??????? ???????: f(x,y) = 100+7x+5y-10xy+3x^2+10y^2, ??????? ???????????? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test4(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 4 -----------------------------------\n\n";
    fout_txt << "04. ??????? ???????: f(x,y) = 100+7x+5y-10.95xy+3x^2+10y^2, ??????? ???????????? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test5(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 5 -----------------------------------\n\n";
    fout_txt << "05. ??????? ???????: f(x,y,z) = 1+x+y+z+xy+xz+yz+x^2+y^2+z^2, ??????? ???????????? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test6(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 6 -----------------------------------\n\n";
    fout_txt << "06. ??????? ???????: f(x,y) = 10x^4+15y^4+15xy, ??????? ??? ?????????? ????????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test7(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 7 -----------------------------------\n\n";
    fout_txt << "07. ??????? ???????: f(x,y) = 10x^6+15y^6-20x^3y+xy^3, ??????? ??? ?????????? ????????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test8(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 8 -----------------------------------\n\n";
    fout_txt << "08. ??????? ???????: f(x,y) = x^6+y^6-2(x^3y+xy^3)+x^2+y^2, ??????? ??? ?????????? ????????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test9(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 9 -----------------------------------\n\n";
    fout_txt << "09. ??????? ???????: f(x,y) = x^6+y^6-3(x^3y+xy^3)+x^2+y^2, ??????? ??? ?????????? ???????? ? ???? ?????????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test10(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 10 ----------------------------------\n\n";
    fout_txt << "10. ??????? ???????: f(x,y) = x^6+y^6-2(x^3y+xy^3)+x^4+y^4-x^2-y^2, ??????? ??? ?????????? ???????? ? ??? ?????????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test11(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 11 ----------------------------------\n\n";
    fout_txt << "11. ??????? ??????? ???????????: f(x,y) = (x^2+y-11)^2+(x+y^2-7)^2, ??????? ?????? ?????????? ???????? ? ???? ?????????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test15(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 15 ----------------------------------\n\n";
    fout_txt << "15. ??????? ??????? ???????: f(x,y)=0.26(x^2+y^2)-0.48xy, ??????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test18(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 18 ----------------------------------\n\n";
    fout_txt << "18. ??????? ??????? ??????: f(x,y)=x/4+(x^2-2x+y^2)^2, ??????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test19(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 19 ----------------------------------\n\n";
    fout_txt << "19. ??????? ??????? ????: f(x,y)=(xy-x+1.5)^2+(xy^2-x+2.25)^2+(x*y^3-x+2.625)^2, ??????? ??? ?????????? ????????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test20(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 20 ----------------------------------\n\n";
    fout_txt << "20. ??????? ???????: f(x,y,z,t) = (x-1)^2+100(x^2-y)^2+10.1(y-1)^2+(z-1)^2+90(z^2-t)^2+10.1(t-1)^2+19.8(t-1)/y, ????? ??????? ?? -10 < x_i < 10. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test21(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 21 ----------------------------------\n\n";
    fout_txt << "21. ??????? ???????: f(x,y) = (y-x^2)^2+(1-x)^2, ????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test22(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 22 ----------------------------------\n\n";
    fout_txt << "22. ??????? ???????: f(x,y) = (y-x^2)^2+100(1-x)^2, ????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test23(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 23 ----------------------------------\n\n";
    fout_txt << "23. ??????? ???????: f(x,y) = 100(y-x^3)^2+(1-x)^2, ????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test24(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 24----------------------------------\n\n";
    fout_txt << "24. ??????? ???????: f(x,y,z,t) = (x+10y)^2+5(z-t)^2+(y-2z)^4+10(x-t)^4, ????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test25(Method method) {
    ...
    fout_txt << "----------------------------------- ???? 25 ----------------------------------\n\n";
    fout_txt << "25. ??????? ???????: f(x,y,z,t) = (x^2-y+1)^4+100(y-z)^6+tg^4(z-t)+x^8+(t-1)^2, ????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test27_2(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 27_2 ----------------------------------\n\n";
    fout_txt << "27. ??????? ??????? ????????? ????????-?????????? 1:\n\tf(x1, ..., xn) = (x1-1)^2/4+sum_(i=1)^(n-1)(x_(i+1)-2*x_i^2+1)^2, ??????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test28_2(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 28_2 ----------------------------------\n\n";
    fout_txt << "28. ??????? ??????? ??????????:\n\tf(x1, ..., xn) = sum_(i=1)^(n-1) (100(x_i^2-x_(i+1))^2+(x_i-1)^2), ??????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
...
void test30_2(Method method) {
    ...
    auto start_points = gen_start_points(2, -5, 5);
    fout_txt << "----------------------------------- ???? 30_2 ----------------------------------\n\n";
    fout_txt << "30. ??????? ???????:\n\tf(x1, ..., xn) = x_1^2+10^6*sum_(i=1)^(n-1) (x_i^2), ??????? ???? ?????????? ???????. \n????????? ? ????????? \"???????? ???????\"\n\n";
    fout_txt << "??????? ????????: iter_counter >= 100 || |f_i-f_(i-1)| < 0.00000001\n\n";
